...
SCAN_DOES="Checks database tables through WordPress's existing database connection, repairs only tables that fail CHECK TABLE, optimizes them, and performs a final verification."
SCAN_WHY="This avoids WP-CLI's external mysqlcheck dependency, so restricted hosts can keep proc_open/proc_close disabled while database maintenance still works."
...
function pw_check_one($wpdb,$table){$wpdb->last_error='';$rows=$wpdb->get_results('CHECK TABLE '.pw_ident($table),ARRAY_A);if($wpdb->last_error)return [false,pw_clean_msg($wpdb->last_error)];if(!is_array($rows)||!$rows)return [false,'CHECK TABLE returned no status'];$ok=false;$problems=[];foreach($rows as $row){$type=strtolower((string)($row['Msg_type']??$row['msg_type']??''));$text=(string)($row['Msg_text']??$row['msg_text']??'');if($type==='status'&&strtolower(trim($text))==='ok'){$ok=true;continue;}if($type==='error')$problems[]=$text?:'database reported an error';}if(!$ok||$problems){if(!$problems)foreach($rows as $row){$type=(string)($row['Msg_type']??$row['msg_type']??'status');$text=(string)($row['Msg_text']??$row['msg_text']??'unknown response');$problems[]=$type.': '.$text;}return [false,pw_clean_msg(implode(' | ',$problems))];}return [true,'OK'];}
...
if($action==='check'){$bad=0;foreach($tables as $table){[$ok,$msg]=pw_check_one($wpdb,$table);if(!$ok){$bad++;echo "BAD\t",pw_clean_msg($table),"\t",pw_clean_msg($msg),"\n";}}exit($bad?10:0);}
if($action==='repair'){$unresolved=0;foreach($tables as $table){[$ok,$before]=pw_check_one($wpdb,$table);if($ok)continue;$wpdb->last_error='';$rows=$wpdb->get_results('REPAIR TABLE '.pw_ident($table),ARRAY_A);$err=$wpdb->last_error;[$after_ok,$after]=pw_check_one($wpdb,$table);if($after_ok)echo "REPAIRED\t",pw_clean_msg($table),"\n";else{$unresolved++;$detail=$err?:$after;if(!$detail&&pw_sql_result_has_error($rows))$detail='REPAIR TABLE reported an error';echo "UNRESOLVED\t",pw_clean_msg($table),"\t",pw_clean_msg($detail?:$before),"\n";}}exit($unresolved?10:0);}
...
main() {
  ...
  for s in "${WP_SITES[@]}"; do
    ...
    printf '\n    %s%s%s%s\n' "$B" "$M" "$d" "$X"
    out=$(_db_native "$s" check "$helper" 2>&1); rc=$?
    if [ "$rc" -eq 0 ]; then printf '      %s✓ CHECK%s      healthy\n' "$G" "$X"
    elif [ "$rc" -eq 10 ]; then
      printf '      %s⚠ CHECK%s      table problem detected — attempting SQL repair automatically\n' "$Y" "$X"; _show_db_details "$out"
      ...
      elif [ "$rc" -eq 10 ]; then printf '      %s⚠ REPAIR%s     one or more tables could not be automatically repaired\n' "$Y" "$X"; _show_db_details "$out"
    ...
    if [ "$runtime_failed" -eq 0 ]; then out=$(_db_native "$s" optimize "$helper" 2>&1); rc=$?; if [ "$rc" -eq 0 ]; then optimized_n=$((optimized_n+1)); printf '      %s✓ OPTIMIZE%s   completed\n' "$G" "$X"; elif [ "$rc" -eq 10 ]; then optimize_failed=1; printf '      %s⚠ OPTIMIZE%s   one or more tables returned an optimization error\n' "$Y" "$X"; _show_db_details "$out"; else runtime_failed=1; printf '      %s⚠ ENGINE%s     OPTIMIZE could not run; tooling/bootstrap error\n' "$Y" "$X"; fi; fi
    if [ "$runtime_failed" -eq 0 ]; then out=$(_db_native "$s" check "$helper" 2>&1); rc=$?; if [ "$rc" -eq 0 ]; then printf '      %s✓ VERIFY%s     healthy after maintenance\n' "$G" "$X"; elif [ "$rc" -eq 10 ]; then final_failed=1; failed_n=$((failed_n+1)); printf '      %s✖ VERIFY%s     database table check still reports an actual problem\n' "$R" "$X"; _show_db_details "$out"; else runtime_failed=1; printf '      %s⚠ ENGINE%s     final verification could not run; DB health is unknown\n' "$Y" "$X"; fi; fi
    ...
    if [ "$runtime_failed" -eq 1 ]; then runtime_failed_n=$((runtime_failed_n+1)); flag "$d" "database maintenance engine could not complete; database health not classified"; elif [ "$final_failed" -eq 1 ]; then issue "$d" "database table(s) remain unhealthy after maintenance"; elif [ "$optimize_failed" -eq 1 ]; then flag "$d" "database verifies clean, but one or more tables could not be optimized"; elif [ "$repaired" -eq 1 ]; then printf '      %s%s✓ RESULT%s    repaired + optimized + verified\n' "$B" "$G" "$X"; else printf '      %s%s✓ RESULT%s    healthy + optimized + verified\n' "$B" "$G" "$X"; fi
  done
  printf '\n    %sSUMMARY%s  repaired: %s%s%s  • optimized: %s%s%s  • unresolved: %s%s%s  • engine errors: %s%s%s\n' "$B$C" "$X" "$B" "$repaired_n" "$X" "$B" "$optimized_n" "$X" "$B" "$failed_n" "$X" "$B" "$runtime_failed_n" "$X"
